...
CError CNoneSyntaxValidator::Validate(C_RETURN_SYNTAX& syntax, C_CONTEXT& context) const
{
  CError result;

  if (syntax.GetExpressionID() == NONE_ID)
  {
    result.SetSource(EErrorSource::Producing);
    result.SetLiveLine(syntax.GetLiveLine());
    result.SetDescription(L"Indexless express syntax tree when return.");
  }
